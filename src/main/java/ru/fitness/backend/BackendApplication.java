...
public class BackendApplication {
	...
	@Bean
	public CommandLineRunner demoData(UserRepository userRepository, PasswordEncoder passwordEncoder, WorkoutTypeRepository workoutTypeRepository) {
		return args -> {
			...
			if (adminOptional.isEmpty()) {
				...
				if (!passwordEncoder.matches("password", admin.getPassword())) {
					System.out.println("Пароль админа обновлен на дефолтный.");
					admin.setPassword(passwordEncoder.encode("password"));
					userRepository.save(admin);
				}
			}
			...
			Optional<User> userOptional = userRepository.findByEmail("user@fitness.com");
			if (userOptional.isEmpty()) {
				System.out.println("Пользователь user@fitness.com не найден, создаем...");
				User user = new User();
				user.setEmail("user@fitness.com");
				...
				System.out.println("Пользователь user@fitness.com создан.");
			}

			// Добавление типов тренировок, если они отсутствуют
			if (workoutTypeRepository.count() == 0) {
